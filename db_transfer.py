...
class CryoETDB:
    ...
    def transfer(self, proc_dir: str | Path, subframe_path: Optional[str | Path]=None) -> int:
        ...
        for dataset in sorted(self.completed):
            ...
            if subframe_path:
                ...
                print("NO SUBFRAME PATH. FRAMES WILL NOT BE TRANSFERRED TO THE DATABASE")
            ...
            print(f"TRANSFERRING DATASET FOR {row['dataset']} -> {row['id']}")
            ...
            if subframe_path:
                ...
                if (exit_code := proc_frames.returncode) != 0:
                    ...
                    print(f"ERROR transfering frames for {row['id']}")
            ...
            if (exit_code := proc_dataset.returncode) != 0:
                ...
                self.dataset_logger.error('ERROR CODE %d' % exit_code)
                print(f"ERROR transferring processed data for {row['id']}")
                print(f"Error code {exit_code}")
            ...
            print(f"MOVING DATASET {row['dataset']} TO {done_dir}")
            if (exit_code := subprocess.run(cmd, shell=True).returncode) != 0:
                ...
                self.dataset_logger.error('ERROR CODE %d' % exit_code)
                print(f"ERROR moving dataset {row['dataset']} to {done_dir}")
                print(f"Error code {exit_code}")
    ...
    def _transfer_rawframes(
        ...
        if (exit_code := subprocess.run(cmd, shell=True).returncode) != 0:
            ...
            self.dataset_logger.error('Error code %d' % exit_code)
            print(f"ERROR creating frames directory {transfer_path}/Frames in the database")
            print(f"Error code {exit_code}")
            return exit_code  
            ...
            for line in f:
                if 'SubFramePath' in line:
                    ...
                    if ext == 'mrc':
                        ...
                        print(f"ERROR: Do not recognize frames file extenstion .{ext}")
                        return 1
